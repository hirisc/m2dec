...
def pattern_to_code(pattern)
  pat = pattern.gsub(/\s/, "").gsub(/-/, "0").split(//)
  code = 0
  len = pat.size
  pat.each { |e|
    code = (code << 1) | e.to_i
...
def read_codes(codes, h, idx)
  codes.each { |line|
    val = (line[0].to_i * 32) + line[1].to_i
    t = line[idx + 2]
...
def devide_tables(filename, bit_num, idx)
...
if !ARGV[0]
  ...
  devide_tables(ARGV[0], ARGV[1] ? ARGV[1].to_i : 6, ARGV[2] ? ARGV[2].to_i : 0)
end
