...
static inline int cabac_decode_decision(h264d_cabac_t *cb, dec_bits *st, int ctxIdx)
{
	...
	static const int8_t state_trans0[64] = {
		1, 2, 3, 4, 5, 6, 7, 8,
	...
	static const int8_t state_trans1[64] = {
		0, 0, 1, 2, 2, 4, 4, 5,
		6, 7, 8, 9, 9, 11, 11, 12,
	...
	if (offset < range) {
		binVal = valMPS;
		pStateIdx = state_trans0[pStateIdx] * 2 + valMPS;
	} else {
		binVal = valMPS ^ 1;
		cb->offset = offset = offset - range;
		range = lps;
		valMPS = pStateIdx ? valMPS : valMPS ^ 1;
		pStateIdx = state_trans1[pStateIdx] * 2 + valMPS;
	}
