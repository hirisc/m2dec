...
static inline uint8_t cbf_top(uint32_t cbf_base)
{
	return ((cbf_base >> 14) & 0x60) | ((cbf_base >> 12) & 0x1c) | ((cbf_base >> 10) & 3);
}

static inline uint8_t cbf_left(uint32_t cbf_base)
{
	return ((cbf_base >> 14) & 64) | ((cbf_base >> 13) & 32) | ((cbf_base >> 12) & 0x18) | ((cbf_base >> 11) & 4) | ((cbf_base >> 6) & 2) | ((cbf_base >> 5) & 1);
}
...
void init_cabac_context(h264d_cabac_t *cabac, int slice_qp);
...
static int slice_header(h264d_context *h2d, dec_bits *st)
{
	...
	if ((pps->entropy_coding_mode_flag) && (hdr->first_mb_in_slice == 0)) {
		init_cabac_context(h2d->mb_current.cabac, h2d->mb_current.qp);
	}
...
//static int residual_block_cabac(h264d_mb_current *mb, dec_bits *st, int *coeff, int num_coeff, const int16_t *qmat, int avail, int pos4x4);
...
static int residual_chroma(h264d_mb_current *mb, uint32_t cpb, dec_bits *st, int avail);
...
static void mb_intra_save_info(h264d_mb_current *mb)
{
	...
	mb->lefttop_mv[0] = mb->top4x4inter->mv[3][0];
	mb->lefttop_mv[1] = mb->top4x4inter->mv[3][1];
	memset(mb->left4x4inter, 0, sizeof(*mb->left4x4inter));
	memset(mb->top4x4inter, 0, sizeof(*mb->top4x4inter));
				...
				F4 ResidualBlock)
{
	...
	cbp = CodedBlockPattern(mb, st, mb->cabac, 0, avail);
	luma = mb->luma;
	offset = mb->offset4x4;
	pr = pred4x4;
	if (cbp) {
	...
	if (cbp & 15) {
		...
		if (cbp & 1) {
			...
			if ((c0 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 0) : -1, avail & 2 ? UNPACK(*mb->top4x4coef, 0) : -1, st, coeff, 16, qmat, avail_intra, 0, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[0], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + 4, stride, avail_intra | (avail_intra & 2 ? 5 : 1));
			if ((c1 = ResidualBlock(mb, c0, avail & 2 ? UNPACK(*mb->top4x4coef, 1) : -1, st, coeff, 16, qmat,  avail_intra, 1, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[1], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + offset[2], stride, avail_intra | 6);
			if ((c2 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 1) : -1, c0, st, coeff, 16, qmat,  avail_intra, 2, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[2], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + offset[3], stride, 3);
			if ((c3 = ResidualBlock(mb, c2, c1, st, coeff, 16, qmat,  avail_intra, 3, 2, 0xf)) != 0) {
		...
		if (cbp & 2) {
			...
			if ((c0 = ResidualBlock(mb, c1, avail & 2 ? UNPACK(*mb->top4x4coef, 2) : -1, st, coeff, 16, qmat,  avail_intra, 4, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[4], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + offset[5], stride, avail_intra | 1);
			if ((c1 = ResidualBlock(mb, c0, avail & 2 ? UNPACK(*mb->top4x4coef, 3) : -1, st, coeff, 16, qmat,  avail_intra, 5, 2, 0xf)) != 0) {
				left = PACK(0, c1, 0);
			...
			if ((c4 = ResidualBlock(mb, c3, c0, st, coeff, 16, qmat,  avail_intra, 6, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[6], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + offset[7], stride, 3);
			if ((c5 = ResidualBlock(mb, c4, c1, st, coeff, 16, qmat,  avail_intra, 7, 2, 0xf)) != 0) {
				left = PACK(left, c5, 1);
		...
		if (cbp & 4) {
			...
			if ((c0 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 2) : -1, c2, st, coeff, 16, qmat,  avail_intra, 8, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[8], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + offset[9], stride, 7);
			if ((c1 = ResidualBlock(mb, c0, c3, st, coeff, 16, qmat,  avail_intra, 9, 2, 0xf)) != 0) {
				ac4x4transform_acdc(luma + offset[9], coeff, stride);
			}
			intra4x4pred_func[*pr++](luma + offset[10], stride, avail_intra | 6);
			if ((c2 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 3) : -1, c0, st, coeff, 16, qmat,  avail_intra, 10, 2, 0xf)) != 0) {
				top = PACK(0, c2, 0);
			...
			if ((c3 = ResidualBlock(mb, c2, c1, st, coeff, 16, qmat,  avail_intra, 11, 2, 0xf)) != 0) {
				top = PACK(top, c3, 1);
	...
	VC_CHECK;
	return residual_chroma(mb, cbp >> 4, st, avail);
}
...
struct cbp_intra_cavlc {
	uint32_t operator()(h264d_mb_current *mb, dec_bits *st, h264d_cabac_t *cb, int is_inter, int avail) {
				...
				F2 ResidualBlock)
{
	...
	ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 0) : -1, avail & 2 ? UNPACK(*mb->top4x4coef, 0) : -1, st, coeff, 16, mb->qmaty, avail_intra, 0, 0, 0);
	mb->left4x4coef &= 0xffff0000;
	*mb->top4x4coef &= 0xffff0000;
	mb->left4x4pred = 0x22222222;
	*mb->top4x4pred = 0x22222222;
	...
	store_strength_intra(mb);
	mb_intra_save_info(mb);
	return residual_chroma(mb, mbc->cbp, st, avail);
}
				...
				F2 ResidualBlock)
{
	...
	qmat = mb->qmaty;
	ResidualBlock(mb, na, nb, st, coeff, 16, qmat, avail_intra, 0, 0, 0);
	intra16x16_dc_transform(coeff, dc);
	c0 = ResidualBlock(mb, na, nb, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	offset = mb->offset4x4;
	dcp = dc;
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c1 = ResidualBlock(mb, c0, avail & 2 ? UNPACK(*mb->top4x4coef, 1) : -1, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c2 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 1) : -1, c0, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c3 = ResidualBlock(mb, c2, c1, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);

	c0 = ResidualBlock(mb, c1, avail & 2 ? UNPACK(*mb->top4x4coef, 2) : -1, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c1 = ResidualBlock(mb, c0, avail & 2 ? UNPACK(*mb->top4x4coef, 3) : -1, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	left = mb->left4x4coef & 0xffff0000;
	left = PACK(left, c1, 0);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c4 = ResidualBlock(mb, c3, c0, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c5 = ResidualBlock(mb, c4, c1, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	left = PACK(left, c5, 1);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);

	c0 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 2) : -1, c2, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c1 = ResidualBlock(mb, c0, c3, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c2 = ResidualBlock(mb, avail & 1 ? UNPACK(mb->left4x4coef, 3) : -1, c0, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	...
	c0 = ResidualBlock(mb, c1, c4, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c1 = ResidualBlock(mb, c0, c5, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	left = PACK(left, c1, 2);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c2 = ResidualBlock(mb, c3, c0, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
	top = PACK(top, c2, 2);
	coeff[0] = *dcp++;
	ac4x4transform_acdc(luma + *offset++, coeff, stride);
	c3 = ResidualBlock(mb, c2, c1, st, coeff, 15, qmat, avail_intra, 0, 1, 0x1f);
...
static inline int residual_chroma(h264d_mb_current *mb, uint32_t cbp, dec_bits *st, int avail)
{
	int coeff[16];
	int dc[4 * 2];
	const int16_t *qmat;
	...
	qmat = mb->qmatc_p;
	for (int i = 0; i < 2; ++i) {
		residual_block_cavlc(0, 0, st, coeff, 4, qmat, 0);
		intra_chroma_dc_transform(coeff, &dc[i * 4]);
	}
	chroma = mb->chroma;
	stride = mb->max_x * 16;
	dcp = dc;
...
static int mb_intrapcm(h264d_mb_current *mb, const mb_code *mbc, dec_bits *st, int avail)
{
	...
	*mb->top4x4pred = 0x22222222;
	...
	mb->prev_qp_delta = 0;
	mb->left4x4inter->cbf = 0x7f;
	mb->top4x4inter->cbf = 0x7f;
	mb_intra_save_info(mb);
	return 0;
}
...
static inline void determine_mv(const int16_t *mva, const int16_t *mvb, const int16_t *mvc, int& mvx, int& mvy, int avail, int idx_map)
{
	int pmvx, pmvy;
...
static inline void calc_mv16x16(h264d_mb_current *mb, int& mvx, int& mvy, int ref_idx, int avail)
{
	prev_mb_t *pmb;
	...
	determine_mv(mva, mvb, mvc, mvx, mvy, avail, idx_map);
}

static uint32_t residual_luma_inter(h264d_mb_current *mb, uint32_t cbp, dec_bits *st, int avail);
static inline void inter_pred8x8(h264d_mb_current *mb, int mvx, int mvy, int width, int height, int ref_idx, int offsetx, int offsety);
...
static inline uint32_t str_mv_calc16x16(uint32_t str, int mvx, int mvy, int ref_idx, const prev_mb_t *top)
{
	for (int i = 0; i < 4; ++i) {
		if (!(str & (3 << (i * 2)))) {
			if ((ref_idx != top->ref[i >> 1]) || (16 <= DIF_SQUARE(mvx, top->mv[i][0])) || (16 <= DIF_SQUARE(mvy, top->mv[i][1]))) {
				str |= 1 << (i * 2);
			}
		}
...
static int mb_inter16x16(h264d_mb_current *mb, const mb_code *mbc, dec_bits *st, int avail)
{
	...
	ref_idx = (0 < t) ? te_golomb(st, t) : 0;
	mvx = se_golomb(st);
	mvy = se_golomb(st);
	calc_mv16x16(mb, mvx, mvy, ref_idx, avail);
	inter_pred8x8(mb, mvx, mvy, 16, 16, ref_idx, 0, 0);
	uint32_t left4x4 = mb->left4x4coef;
	uint32_t top4x4 = *mb->top4x4coef;
	cbp = me_golomb(st, 1);
	if (cbp) {
		str_vert = residual_luma_inter(mb, cbp, st, avail);
	...
	if (mb->y != 0) {
		...
		str_vert = str_mv_calc16x16(str_vert, mvx, mvy, ref_idx, mb->top4x4inter);
	}
	...
	if (mb->x != 0) {
		...
		str_horiz = str_mv_calc16x16(str_horiz, mvx, mvy, ref_idx, mb->left4x4inter);
	}
	...
	*mb->top4x4pred = 0x22222222;
	...
	for (int i = 0; i < 4; ++i) {
		mb->top4x4inter->mv[i][0] = mvx;
		mb->top4x4inter->mv[i][1] = mvy;
		mb->left4x4inter->mv[i][0] = mvx;
		mb->left4x4inter->mv[i][1] = mvy;
	}

	return residual_chroma(mb, cbp >> 4, st, avail);
}

static inline void calc_mv16x8top(h264d_mb_current *mb, int& mvx, int& mvy, int ref_idx, int avail)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
...
static inline void calc_mv16x8bottom(h264d_mb_current *mb, int& mvx, int& mvy, int ref_idx, int avail, int prev_ref, const int16_t *prev_mv)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
...
static inline uint32_t str_mv_calc16x8_left(uint32_t str, int ref_idx0, int ref_idx1, int mvx0, int mvy0, int mvx1, int mvy1, const prev_mb_t *top)
{
	int i;
	for (i = 0; i < 2; ++i) {
		if (!(str & (3 << (i * 2))) && ((ref_idx0 != top->ref[0]) || (16 <= DIF_SQUARE(mvx0, top->mv[i][0])) || (16 <= DIF_SQUARE(mvy0, top->mv[i][1])))) {
			str |= 1 << (i * 2);
		}
	}
	for (;i < 4; ++i) {
		int shift = i * 2;
		if (!(str & (3 << shift)) && ((ref_idx1 != top->ref[1]) || (16 <= DIF_SQUARE(mvx1, top->mv[i][0])) || (16 <= DIF_SQUARE(mvy1, top->mv[i][1])))) {
			str |= 1 << shift;
		}
	}
	return str;
...
static inline uint32_t str_mv_calc16x8_vert(uint32_t str, int ref_idx0, int ref_idx1, int mvx0, int mvy0, int mvx1, int mvy1)
{
	if ((ref_idx0 != ref_idx1) || (16 <= DIF_SQUARE(mvx0, mvx1)) || (16 <= DIF_SQUARE(mvy0, mvy1))) {
		for (int i = 0; i < 4; ++i) {
			int shift = i * 2 + 16;
			if (!(str & (3 << shift))) {
				str |= 1 << shift;
...
static int mb_inter16x8(h264d_mb_current *mb, const mb_code *mbc, dec_bits *st, int avail)
{
	...
	t = *mb->num_ref_idx_l0_active_minus1;
	if (0 < t) {
		ref_idx0 = te_golomb(st, t);
		ref_idx1 = te_golomb(st, t);
	} else {
		ref_idx0 = 0;
		ref_idx1 = 0;
	}
	mvx0 = se_golomb(st);
	mvy0 = se_golomb(st);
	calc_mv16x8top(mb, mvx0, mvy0, ref_idx0, avail);
	inter_pred8x8(mb, mvx0, mvy0, 16, 8, ref_idx0, 0, 0);
	mvx1 = se_golomb(st);
	mvy1 = se_golomb(st);
	mv0[0] = mvx0;
	mv0[1] = mvy0;
	calc_mv16x8bottom(mb, mvx1, mvy1, ref_idx1, avail, ref_idx0, mv0);
	inter_pred8x8(mb, mvx1, mvy1, 16, 8, ref_idx1, 0, 8);
	...
	if (mb->y != 0) {
		...
		str_vert = str_previous_coef(str_vert, top4x4);
		str_vert = str_mv_calc16x16(str_vert, mvx0, mvy0, ref_idx0, mb->top4x4inter);
	}
	deb->str_vert = str_mv_calc16x8_vert(str_vert, ref_idx0, ref_idx1, mvx0, mvy0, mvx1, mvy1);
	if (mb->x != 0) {
		if (mb->left4x4inter->type <= MB_IPCM) {
			deb->str4_horiz = 1;
			str_horiz |= 1;
		...
		str_horiz = str_previous_coef(str_horiz, left4x4);
		str_horiz = str_mv_calc16x8_left(str_horiz, ref_idx0, ref_idx1, mvx0, mvy0, mvx1, mvy1, mb->left4x4inter);
	}
	deb->str_horiz = str_horiz;
	...
	*mb->top4x4pred = 0x22222222;
	...
	mb->lefttop_mv[1] = mb->top4x4inter->mv[3][1];
	for (int i = 0; i < 4; ++i) {
		mb->top4x4inter->mv[i][0] = mvx1;
		mb->top4x4inter->mv[i][1] = mvy1;
	}
	mb->top4x4inter->ref[0] = ref_idx1;
	mb->top4x4inter->ref[1] = ref_idx1;
	for (int i = 0; i < 2; ++i) {
		mb->left4x4inter->mv[i][0] = mvx0;
		mb->left4x4inter->mv[i][1] = mvy0;
		mb->left4x4inter->mv[2 + i][0] = mvx1;
		mb->left4x4inter->mv[2 + i][1] = mvy1;
	}
	mb->left4x4inter->ref[0] = ref_idx0;
	mb->left4x4inter->ref[1] = ref_idx1;
	return residual_chroma(mb, cbp >> 4, st, avail);
}

static inline void calc_mv8x16left(h264d_mb_current *mb, int& mvx, int& mvy, int ref_idx, int avail)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
...
static inline void calc_mv8x16right(h264d_mb_current *mb, int& mvx, int& mvy, int ref_idx, int avail, int prev_ref, const int16_t *prev_mv)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
...
static int mb_inter8x16(h264d_mb_current *mb, const mb_code *mbc, dec_bits *st, int avail)
{
	...
	mvx0 = se_golomb(st);
	mvy0 = se_golomb(st);
	calc_mv8x16left(mb, mvx0, mvy0, ref_idx0, avail);
	inter_pred8x8(mb, mvx0, mvy0, 8, 16, ref_idx0, 0, 0);
	mvx1 = se_golomb(st);
	mvy1 = se_golomb(st);
	mv0[0] = mvx0;
	mv0[1] = mvy0;
	calc_mv8x16right(mb, mvx1, mvy1, ref_idx1, avail, ref_idx0, mv0);
	inter_pred8x8(mb, mvx1, mvy1, 8, 16, ref_idx1, 8, 0);
	VC_CHECK;
	...
	if (mb->y != 0) {
		...
		str_vert = str_previous_coef(str_vert, top4x4);
		str_vert = str_mv_calc16x8_left(str_vert, ref_idx0, ref_idx1, mvx0, mvy0, mvx1, mvy1, mb->top4x4inter); /* same as 16x8 left */
	}
	deb->str_vert = str_vert;
	if (mb->x != 0) {
		if (mb->left4x4inter->type <= MB_IPCM) {
		...
		str_horiz = str_previous_coef(str_horiz, left4x4);
		str_horiz = str_mv_calc16x16(str_horiz, mvx0, mvy0, ref_idx0, mb->left4x4inter);
	}
	deb->str_horiz = str_mv_calc16x8_vert(str_horiz, ref_idx0, ref_idx1, mvx0, mvy0, mvx1, mvy1); /* same as 16x8 vert */
	...
	*mb->top4x4pred = 0x22222222;
	...
	mb->lefttop_mv[1] = mb->top4x4inter->mv[3][1];
	for (int i = 0; i < 2; ++i) {
		mb->top4x4inter->mv[i][0] = mvx0;
		mb->top4x4inter->mv[i][1] = mvy0;
		mb->top4x4inter->mv[2 + i][0] = mvx1;
		mb->top4x4inter->mv[2 + i][1] = mvy1;
	}
	mb->top4x4inter->ref[0] = ref_idx0;
	mb->top4x4inter->ref[1] = ref_idx1;
	for (int i = 0; i < 4; ++i) {
		mb->left4x4inter->mv[i][0] = mvx1;
		mb->left4x4inter->mv[i][1] = mvy1;
	}
	mb->left4x4inter->ref[0] = ref_idx1;
	mb->left4x4inter->ref[1] = ref_idx1;
	return residual_chroma(mb, cbp >> 4, st, avail);
}
...
static inline void calc_mv8x8_sub8x8(h264d_mb_current *mb, int& mvx, int& mvy, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
	...
	if (blk_idx & 2) {
		...
		mvb = pblk[blk_idx - 2].mv[2];
		avail |= 2;
	} else if (avail & 2) {
		pmb = mb->top4x4inter;
		idx_map |= (ref_idx == pmb->ref[blk_idx]) * 2;
		mvb = pmb->mv[blk_idx * 2];
	} else {
		mvb = zero_mv;
	}
...
static inline void calc_mv8x8_sub8x4(h264d_mb_current *mb, int& mvx, int& mvy, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk, int y)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
	...
	if (blk_idx & 1) {
		idx_map = (ref_idx == pblk[blk_idx - 1].ref);
		mva = pblk[blk_idx - 1].mv[y * 2 + 1];
		avail |= 1;
	} else if (avail & 1) {
		pmb = mb->left4x4inter;
		idx_map = (ref_idx == pmb->ref[blk_idx >> 1]);
		mva = pmb->mv[(blk_idx & 2) + y];
	} else {
		idx_map = 0;
		mva = zero_mv;
	}
...
static inline void calc_mv8x8_sub4x8(h264d_mb_current *mb, int& mvx, int& mvy, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk, int x)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
	...
	if (x == 1) {
		...
		pmb = mb->left4x4inter;
		idx_map = (ref_idx == pmb->ref[blk_idx >> 1]);
		mva = pmb->mv[blk_idx]; /* blk_idx shall be 0 or 2 */
	} else {
		idx_map = 0;
		mva = zero_mv;
	}
	...
	if (blk_idx & 2) {
		idx_map |= (ref_idx == pblk[blk_idx - 2].ref) * 2;
		mvb = pblk[blk_idx - 2].mv[2 + x];
		avail |= 2;
	} else if (avail & 2) {
		pmb = mb->top4x4inter;
		idx_map |= (ref_idx == pmb->ref[blk_idx & 1]) * 2;
		mvb = pmb->mv[blk_idx * 2 + x];
	} else {
		mvb = zero_mv;
	}
...
static inline void calc_mv8x8_sub4x4(h264d_mb_current *mb, int& mvx, int& mvy, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk, int xy)
{
	prev_mb_t *pmb;
	const int16_t *mva, *mvb, *mvc;
	int idx_map;
	...
	if (xy & 1) {
		idx_map = 1;
		mva = pblk[blk_idx].mv[xy - 1];
		avail |= 1;
	} else if (blk_idx & 1) {
		idx_map = (ref_idx == pblk[blk_idx - 1].ref);
		mva = pblk[blk_idx - 1].mv[xy + 1]; /* xy shall be 0 or 2 */
		avail |= 1;
	} else if (avail & 1) {
		pmb = mb->left4x4inter;
		idx_map = (ref_idx == pmb->ref[blk_idx >> 1]);
		mva = pmb->mv[blk_idx + (xy >> 1)]; /* blk_idx shall be 0 or 2 */
	} else {
		idx_map = 0;
		mva = zero_mv;
	}
	...
	if (xy & 2) {
		idx_map |= 2;
		mvb = pblk[blk_idx].mv[xy - 2];
		avail |= 2;
	} else if (blk_idx & 2) {
		idx_map |= (ref_idx == pblk[blk_idx - 2].ref) * 2;
		mvb = pblk[blk_idx - 2].mv[2 + (xy & 1)];
		avail |= 2;
	} else if (avail & 2) {
		pmb = mb->top4x4inter;
		idx_map |= (ref_idx == pmb->ref[blk_idx & 1]) * 2;
		mvb = pmb->mv[blk_idx * 2 + (xy & 1)];
	} else {
		mvb = zero_mv;
	}
	...
	case 0:
		...
		case 2:
			idx_map |= 4;
			avail |= 4;
			mvc = pblk[blk_idx].mv[1];
			break;
		case 3:
			idx_map |= 4;
			avail |= 4;
			mvc = pblk[blk_idx].mv[0];
			break;
		}
		break;
	case 1:
...
static void sub_mb8x8(h264d_mb_current *mb, dec_bits *st, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk)
{
	...
	mvx = se_golomb(st);
	mvy = se_golomb(st);
	calc_mv8x8_sub8x8(mb, mvx, mvy, avail, ref_idx, blk_idx, pblk);
	pblk += blk_idx;
	pblk->ref = ref_idx;
	for (int i = 0; i < 4; ++i) {
		pblk->mv[i][0] = mvx;
	...
	inter_pred8x8(mb, mvx, mvy, 8, 8, ref_idx, (blk_idx & 1) * 8, (blk_idx & 2) * 4);
...
static void sub_mb8x4(h264d_mb_current *mb, dec_bits *st, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk)
{
	...
	for (int y = 0; y < 2; ++y) {
		mvx = se_golomb(st);
		mvy = se_golomb(st);
		calc_mv8x8_sub8x4(mb, mvx, mvy, avail, ref_idx, blk_idx, pblk, y);
		p->mv[y * 2][0] = mvx;
		p->mv[y * 2][1] = mvy;
		p->mv[y * 2 + 1][0] = mvx;
		p->mv[y * 2 + 1][1] = mvy;
		inter_pred8x8(mb, mvx, mvy, 8, 4, ref_idx, (blk_idx & 1) * 8, ((blk_idx & 2) + y) * 4);
	}
}
...
static void sub_mb4x8(h264d_mb_current *mb, dec_bits *st, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk)
{
	...
	for (int x = 0; x < 2; ++x) {
		mvx = se_golomb(st);
		mvy = se_golomb(st);
		calc_mv8x8_sub4x8(mb, mvx, mvy, avail, ref_idx, blk_idx, pblk, x);
		p->mv[x][0] = mvx;
		p->mv[x][1] = mvy;
		p->mv[x + 2][0] = mvx;
		p->mv[x + 2][1] = mvy;
		inter_pred8x8(mb, mvx, mvy, 4, 8, ref_idx, (blk_idx & 1) * 8 + x * 4, (blk_idx & 2) * 4);
	}
}
...
static void sub_mb4x4(h264d_mb_current *mb, dec_bits *st, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk)
{
	...
	for (int xy = 0; xy < 4; ++xy) {
		mvx = se_golomb(st);
		mvy = se_golomb(st);
		calc_mv8x8_sub4x4(mb, mvx, mvy, avail, ref_idx, blk_idx, pblk, xy);
		p->mv[xy][0] = mvx;
		p->mv[xy][1] = mvy;
		inter_pred8x8(mb, mvx, mvy, 4, 4, ref_idx, (blk_idx & 1) * 8 + (xy & 1) * 4, (blk_idx & 2) * 4 + (xy & 2) * 2);
	}
}

static void (* const sub_mb[4])(h264d_mb_current *mb, dec_bits *st, int avail, int ref_idx, int blk_idx, prev8x8_t *pblk) = {
	sub_mb8x8,
	sub_mb8x4,
	sub_mb4x8,
	sub_mb4x4,
...
static uint32_t str_mv_calc8x8_top(uint32_t str, const prev8x8_t *p, const prev_mb_t *top)
{
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		if (!(str & (3 << (i * 2))) && ((p[i8x8].ref != top->ref[i8x8]) || (16 <= DIF_SQUARE(p[i8x8].mv[i & 1][0], top->mv[i][0])) || (16 <= DIF_SQUARE(p[i8x8].mv[i & 1][1], top->mv[i][1])))) {
			str |= 1 << (i * 2);
		}
	}
	return str;
...
static uint32_t str_mv_calc8x8_vert(uint32_t str, const prev8x8_t *p)
{
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		int shift = i * 2 + 8;
		if (!(str & (3 << shift)) && ((16 <= DIF_SQUARE(p[i8x8].mv[i & 1][0], p[i8x8].mv[(i & 1) + 2][0])) || (16 <= DIF_SQUARE(p[i8x8].mv[i & 1][1], p[i8x8].mv[(i & 1) + 2][1])))) {
			str |= (1 << shift);
		}
	}
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		int shift = i * 2 + 16;
		if (!(str & (3 << shift)) && ((p[i8x8].ref != p[i8x8 + 2].ref) || (16 <= DIF_SQUARE(p[i8x8].mv[(i & 1) + 2][0], p[i8x8 + 2].mv[i & 1][0])) || (16 <= DIF_SQUARE(p[i8x8].mv[(i & 1) + 2][1], p[i8x8 + 2].mv[i & 1][1])))) {
			str |= (1 << shift);
		}
	}
	for (int i = 0; i < 4; ++i) {
...
static uint32_t str_mv_calc8x8_left(uint32_t str, const prev8x8_t *p, const prev_mb_t *left)
{
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		if (!(str & (3 << (i * 2))) && ((p[i & 2].ref != left->ref[i8x8]) || (16 <= DIF_SQUARE(p[i & 2].mv[(i & 1) * 2][0], left->mv[i][0])) || (16 <= DIF_SQUARE(p[i & 2].mv[(i & 1) * 2][1], left->mv[i][1])))) {
			str |= 1 << (i * 2);
		}
	}
	return str;
...
static uint32_t str_mv_calc8x8_horiz(uint32_t str, const prev8x8_t *p)
{
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		int shift = i * 2 + 8;
		if (!(str & (3 << shift)) && ((16 <= DIF_SQUARE(p[i & 2].mv[(i & 1) * 2][0], p[i & 2].mv[(i & 1) * 2 + 1][0])) || (16 <= DIF_SQUARE(p[i & 2].mv[(i & 1) * 2][1], p[i & 2].mv[(i & 1) * 2 + 1][1])))) {
			str |= (1 << shift);
		}
	}
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		int shift = i * 2 + 16;
		if (!(str & (3 << shift)) && ((p[i & 2].ref != p[(i & 2) + 1].ref) || (16 <= DIF_SQUARE(p[i & 2].mv[(i & 1) * 2 + 1][0], p[(i & 2) + 1].mv[(i & 1) * 2][0])) || (16 <= DIF_SQUARE(p[i & 2].mv[(i & 1) * 2 + 1][1], p[(i & 2) + 1].mv[(i & 1) * 2][1])))) {
			str |= (1 << shift);
		}
	}
	for (int i = 0; i < 4; ++i) {
		int i8x8 = i >> 1;
		int shift = i * 2 + 24;
		if (!(str & (3 << shift)) && ((16 <= DIF_SQUARE(p[(i & 2) + 1].mv[(i & 1) * 2][0], p[(i & 2) + 1].mv[(i & 1) * 2 + 1][0])) || (16 <= DIF_SQUARE(p[(i & 2) + 1].mv[(i & 1) * 2][1], p[(i & 2) + 1].mv[(i & 1) * 2 + 1][1])))) {
			str |= (1 << shift);
		}
	}
	return str;
...
static int mb_inter8x8(h264d_mb_current *mb, const mb_code *mbc, dec_bits *st, int avail)
{
	...
	if (t && (mb->type != 30)) {
		for (int i = 0; i < 4; ++i) {
			ref_idx[i] = te_golomb(st, t);
		}
	} else {
		memset(ref_idx, 0, sizeof(ref_idx));
	}
	for (int i = 0; i < 4; ++i) {
		sub_mb[sub_mb_type[i]](mb, st, avail, ref_idx[i], i, curr_blk);
	}
	cbp = me_golomb(st, 1);
	left4x4 = mb->left4x4coef;
	top4x4 = *mb->top4x4coef;
	...
	*mb->top4x4pred = 0x22222222;
	...
	mb->top4x4inter->mv[0][0] = curr_blk[2].mv[2][0];
	mb->top4x4inter->mv[0][1] = curr_blk[2].mv[2][1];
	mb->top4x4inter->mv[1][0] = curr_blk[2].mv[3][0];
	mb->top4x4inter->mv[1][1] = curr_blk[2].mv[3][1];
	mb->top4x4inter->ref[1] = curr_blk[3].ref;
	mb->top4x4inter->mv[2][0] = curr_blk[3].mv[2][0];
	mb->top4x4inter->mv[2][1] = curr_blk[3].mv[2][1];
	mb->top4x4inter->mv[3][0] = curr_blk[3].mv[3][0];
	mb->top4x4inter->mv[3][1] = curr_blk[3].mv[3][1];

	mb->left4x4inter->ref[0] = curr_blk[1].ref;
	mb->left4x4inter->mv[0][0] = curr_blk[1].mv[1][0];
	mb->left4x4inter->mv[0][1] = curr_blk[1].mv[1][1];
	mb->left4x4inter->mv[1][0] = curr_blk[1].mv[3][0];
	mb->left4x4inter->mv[1][1] = curr_blk[1].mv[3][1];
	mb->left4x4inter->ref[1] = curr_blk[3].ref;
	mb->left4x4inter->mv[2][0] = curr_blk[3].mv[1][0];
	mb->left4x4inter->mv[2][1] = curr_blk[3].mv[1][1];
	mb->left4x4inter->mv[3][0] = curr_blk[3].mv[3][0];
	mb->left4x4inter->mv[3][1] = curr_blk[3].mv[3][1];
...
static uint32_t residual_luma_inter(h264d_mb_current *mb, uint32_t cbp, dec_bits *st, int avail)
{
	...
	READ_SE_RANGE(qp_delta, st, -26, 25);
	if (qp_delta) {
		set_qp(mb, mb->qp + qp_delta);
	}
	qmat = mb->qmaty;
	...
	str_map = 0;
	if (cbp & 1) {
		if ((c0 = residual_block_cavlc(avail & 1 ? UNPACK(mb->left4x4coef, 0) : -1, avail & 2 ? UNPACK(*mb->top4x4coef, 0) : -1, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[0], coeff, stride);
			str_map = 0x2;
		}
		if ((c1 = residual_block_cavlc(c0, avail & 2 ? UNPACK(*mb->top4x4coef, 1) : -1, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[1], coeff, stride);
			str_map |= 0x8;
		}
		if ((c2 = residual_block_cavlc(avail & 1 ? UNPACK(mb->left4x4coef, 1) : -1, c0, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[2], coeff, stride);
			str_map |= 0x200;
		}
		if ((c3 = residual_block_cavlc(c2, c1, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[3], coeff, stride);
			str_map |= 0x800;
		}
	} else {
	...
	if (cbp & 2) {
		if ((c0 = residual_block_cavlc(c1, avail & 2 ? UNPACK(*mb->top4x4coef, 2) : -1, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[4], coeff, stride);
			str_map |= 0x20;
		}
		if ((c1 = residual_block_cavlc(c0, avail & 2 ? UNPACK(*mb->top4x4coef, 3) : -1, st, coeff, 16, qmat, 0xf)) != 0) {
			left = PACK(0, c1, 0);
			str_map |= 0x80;
			ac4x4transform_acdc(luma + offset[5], coeff, stride);
		} else {
		...
		if ((c4 = residual_block_cavlc(c3, c0, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[6], coeff, stride);
			str_map |= 0x2000;
		}
		if ((c5 = residual_block_cavlc(c4, c1, st, coeff, 16, qmat, 0xf)) != 0) {
			left = PACK(left, c5, 1);
			str_map |= 0x8000;
			ac4x4transform_acdc(luma + offset[7], coeff, stride);
		}
	...
	if (cbp & 4) {
		if ((c0 = residual_block_cavlc(avail & 1 ? UNPACK(mb->left4x4coef, 2) : -1, c2, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[8], coeff, stride);
			str_map |= 0x20000;
		}
		if ((c1 = residual_block_cavlc(c0, c3, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[9], coeff, stride);
			str_map |= 0x80000;
		}
		if ((c2 = residual_block_cavlc(avail & 1 ? UNPACK(mb->left4x4coef, 3) : -1, c0, st, coeff, 16, qmat, 0xf)) != 0) {
			top = PACK(0, c2, 0);
			str_map |= 0x2000000;
			ac4x4transform_acdc(luma + offset[10], coeff, stride);
		} else {
	...
	if (cbp & 8) {
		if ((c0 = residual_block_cavlc(c1, c4, st, coeff, 16, qmat, 0xf)) != 0) {
			ac4x4transform_acdc(luma + offset[12], coeff, stride);
			str_map |= 0x200000;
		}
		if ((c1 = residual_block_cavlc(c0, c5, st, coeff, 16, qmat, 0xf)) != 0) {
			left = PACK(left, c1, 2);
			str_map |= 0x800000;
			ac4x4transform_acdc(luma + offset[13], coeff, stride);
		}
		if ((c2 = residual_block_cavlc(c3, c0, st, coeff, 16, qmat, 0xf)) != 0) {
			top = PACK(top, c2, 2);
			str_map |= 0x20000000;
			ac4x4transform_acdc(luma + offset[14], coeff, stride);
		}
...
static const mb_code mb_decode[] = {
	...
	{mb_intra16x16_dconly_cavlc, mb_intra16xpred_vert<16>, 1},
	{mb_intra16x16_dconly_cavlc, mb_intra16x16pred_horiz, 1},
	{mb_intra16x16_dconly_cavlc, mb_intra16x16pred_dc, 1},
	{mb_intra16x16_dconly_cavlc, mb_intra16x16pred_planer, 1},
	{mb_intra16x16_dconly_cavlc, mb_intra16xpred_vert<16>, 2},
	{mb_intra16x16_dconly_cavlc, mb_intra16x16pred_horiz, 2},
	{mb_intra16x16_dconly_cavlc, mb_intra16x16pred_dc, 2},
	{mb_intra16x16_dconly_cavlc, mb_intra16x16pred_planer, 2},
	{mb_intra16x16_acdc_cavlc, mb_intra16xpred_vert<16>, 0},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_horiz, 0},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_dc, 0},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_planer, 0},
	{mb_intra16x16_acdc_cavlc, mb_intra16xpred_vert<16>, 1},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_horiz, 1},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_dc, 1},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_planer, 1},
	{mb_intra16x16_acdc_cavlc, mb_intra16xpred_vert<16>, 2},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_horiz, 2},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_dc, 2},
	{mb_intra16x16_acdc_cavlc, mb_intra16x16pred_planer, 2},
...
static int adjust_mb_type(int mb_type, int slice_type)
{
	if (slice_type == I_SLICE) {
		...
		/* B_SLICE (FIXME: SP, SI slice included) */
		mb_type -= 23;
...
static inline int macroblock_layer_cabacI(h264d_mb_current *mb, int slice_type, dec_bits *st);
...
static void calc_mv_pskip(h264d_mb_current *mb, int& mvx, int& mvy, int avail)
{
	prev_mb_t *pmb;
	...
	calc_mv16x16(mb, mvx, mvy, 0, avail);
}

static int skip_mbs(h264d_mb_current *mb, uint32_t skip_mb_num)
{
	...
	do {
		...
		int mvx = 0;
		int mvy = 0;
		calc_mv_pskip(mb, mvx, mvy, get_availability(mb));
		inter_pred8x8(mb, mvx, mvy, 16, 16, 0, 0, 0);
		*mb->top4x4pred = 0x22222222;
		top4x4 = *mb->top4x4coef;
		*mb->top4x4coef = 0;
		...
		if (mb->y != 0) {
			...
			str_vert = str_previous_coef(str_vert, top4x4);
			str_vert = str_mv_calc16x16(str_vert, mvx, mvy, 0, mb->top4x4inter);
		}
		deb->str_vert = str_vert;
		if (mb->x != 0) {
			if (mb->left4x4inter->type <= MB_IPCM) {
			...
			str_horiz = str_previous_coef(str_horiz, left4x4);
			str_horiz = str_mv_calc16x16(str_horiz, mvx, mvy, 0, mb->left4x4inter);
		}
		deb->str_horiz = str_horiz;
		left4x4 = 0;
...
static inline void deblock_i(h264d_mb_current *mb);
...
static int slice_data(h264d_context *h2d, dec_bits *st)
{
	...
	do {
		...
		if ((hdr->slice_type != I_SLICE)
		    && (hdr->slice_type != SI_SLICE)) {
			if (!is_ae) {
				skip_num = ue_golomb(st);
				if (skip_num) {
					if (skip_mbs(mb, skip_num) < 0) {
						break;
					}
				}
			} else {
#if 0
				if (ae_get(st)) {
					if (skip_mb_cabac(mb, 1) < 0) {
						break;
					}
				}
#endif
			}
			if (!more_rbsp_data(st)) {
				break;
			}
		}
		if (is_ae) {
			macroblock_layer_cabacI(mb, hdr->slice_type, st);
		} else {
...
static const ctx_idx_mn_t ctx_idx_mn_I[460] = {
	/* 0 to 10 */
...
static const int8_t state_trans[2][64]= {
	{
		...
		57, 58, 59, 60, 61, 62, 62, 63 /* 62, 62, 63 */
	},
	{
		0, 0, 1, 2, 2, 4, 4, 5,
		6, 7, 8, 9, 9, 11, 11, 12,
...
void init_cabac_context(h264d_cabac_t *cabac, int slice_qp)
{
	const ctx_idx_mn_t *lut = ctx_idx_mn_I;
...
static inline int next_state(int stateidx, int mps)
{
	return mps ? stateidx : stateidx - 63;
}
...
static inline int cabac_decode_decision(h264d_cabac_t *cb, dec_bits *st, int ctxIdx)
{
	...
	if (offset < range) {
		binVal = valMPS;
		pStateIdx = state_trans[0][pStateIdx];
	} else {
		binVal = valMPS ^ 1;
		cb->offset = offset = offset - range;
		range = lps;
		valMPS = pStateIdx ? valMPS : valMPS ^ 1;
		pStateIdx = state_trans[1][pStateIdx];
	}
	cb->context[ctxIdx] = next_state(pStateIdx, valMPS);
	cb->range = range;
	if (range < 256) {
...
static int cabac_mb_type_I(h264d_mb_current *mb, dec_bits *st, int offset, int avail)
{
	int ctx_idx;
	h264d_cabac_t *cb;
	int mb_type;

	ctx_idx = offset + ((avail & 2) && (mb->top4x4inter->type == MB_INxN)) + ((avail & 1) && (mb->left4x4inter->type == MB_INxN));
	cb = mb->cabac;
	if (!cabac_decode_decision(cb, st, ctx_idx)) {
		return 0; /* INxN */
	} else if (cabac_decode_terminate(cb, st)) {
		return MB_IPCM;
	}
	/* I16x16 */
	mb_type = cabac_decode_decision(cb, st, offset + 3) * 12 + 1;
	if (cabac_decode_decision(cb, st, offset + 4)) {
		mb_type = mb_type + cabac_decode_decision(cb, st, offset + 5) * 4 + 4;
	}
	mb_type += cabac_decode_decision(cb, st, offset + 6) * 2;
	mb_type += cabac_decode_decision(cb, st, offset + 7);
	return mb_type;
}
...
struct intra_chroma_pred_mode_cabac {
	uint32_t operator()(h264d_mb_current *mb, dec_bits *st, h264d_cabac_t *cb, int avail) {
		...
		pred_mode = cabac_decode_decision(cb, st, ctx_idx);
		if (pred_mode) {
			pred_mode = cabac_decode_decision(cb, st, 64 + 3) + 1;
		}
...
struct cbp_cabac {
	uint32_t operator()(h264d_mb_current *mb, dec_bits *st, h264d_cabac_t *cb, int is_inter, int avail) {
		...
		/* luma */
		cbp = cabac_decode_decision(cb, st, 73 + a0 + b0);
		cbp |= cabac_decode_decision(cb, st, 73 + (cbp ^ 1) + b1) * 2;
		cbp |= cabac_decode_decision(cb, st, 73 + ((cbp & 1) ^ 1) * 2 + a1) * 4;
		cbp |= cabac_decode_decision(cb, st, 73 + ((cbp & 2) ^ 2) + (((cbp >> 2) & 1) ^ 1)) * 8;
		/* chroma */
		if (cabac_decode_decision(cb, st, 77 + a2 + b2)) {
			cbp = cbp + cabac_decode_decision(cb, st, 77 + 4 + a3 + b3) * 16 + 16;
		}
		return cbp;
	}
};
...
static int (* const ctxidxinc_cbf[16])(h264d_mb_current *mb, uint32_t cbf, int avail) = {
...
//static int residual_block_cabac(int na, int nb, dec_bits *st, int *coeff, int num_coeff, const int16_t *qmat, uint32_t dc_mask)
	int operator()(h264d_mb_current *mb, int na, int nb, dec_bits *st, int *coeff, int num_coeff, const int16_t *qmat, int avail, int pos4x4, int cat, uint32_t dc_mask)
	{
...
static const mb_code mb_decode_cabac[] = {
	...
	{mb_intra16x16_dconly_cabac, mb_intra16xpred_vert<16>, 1},
	{mb_intra16x16_dconly_cabac, mb_intra16x16pred_horiz, 1},
	{mb_intra16x16_dconly_cabac, mb_intra16x16pred_dc, 1},
	{mb_intra16x16_dconly_cabac, mb_intra16x16pred_planer, 1},
	{mb_intra16x16_dconly_cabac, mb_intra16xpred_vert<16>, 2},
	{mb_intra16x16_dconly_cabac, mb_intra16x16pred_horiz, 2},
	{mb_intra16x16_dconly_cabac, mb_intra16x16pred_dc, 2},
	{mb_intra16x16_dconly_cabac, mb_intra16x16pred_planer, 2},
	{mb_intra16x16_acdc_cabac, mb_intra16xpred_vert<16>, 0},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_horiz, 0},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_dc, 0},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_planer, 0},
	{mb_intra16x16_acdc_cabac, mb_intra16xpred_vert<16>, 1},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_horiz, 1},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_dc, 1},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_planer, 1},
	{mb_intra16x16_acdc_cabac, mb_intra16xpred_vert<16>, 2},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_horiz, 2},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_dc, 2},
	{mb_intra16x16_acdc_cabac, mb_intra16x16pred_planer, 2},
	{0, 0, 0},
};
...
static inline int macroblock_layer_cabacI(h264d_mb_current *mb, int slice_type, dec_bits *st)
{
	...
	avail = get_availability(mb);
	mb->type = mbtype = cabac_mb_type_I(mb, st, 3, avail);
